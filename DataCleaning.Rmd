...
```{r}
#Clases exportados e importados
ggplot(data=cites,aes(x=Class, fill=Exportation.type)) + geom_bar() + xlab("Animales por clase exportadas e importadas") + labs(fill = "Exportation.type")

#Cantidad de especimen exportado por paises
ggplot(data=subset(cites, Exportation.type == "Exported"),aes(x=Importer, fill=Importer)) + geom_bar(binwidth=1.0) + xlab("Paises Exportadores")
...
animals_counts <- cites %>%
                 group_by(App., Taxon) %>%
                 filter(Exportation.type=="Exported") %>%
                 summarise(Exportation.number)
                 

#Animales protegidos
ggplot(data=animals_counts, aes(x=Exportation.number)) + geom_histogram(bins=10) + xlab("Animales protegidos") + labs(fill = "Taxon")
